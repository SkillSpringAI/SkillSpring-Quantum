...
This is the next recommended follow-up task for the next session.

## Goal

Run through the whole Electron desktop application manually and document the interaction flow so future UI and UX work is based on observed friction instead of guesswork.
...
1. Imports
2. import history
3. Find Imports / retrieval
4. Readable Archive
5. Datasets
6. diagnostics only when relevant
7. any advanced screen only if it is surfaced by ordinary workflow confusion
...
9. open diagnostics only if something seems unclear, missing, or fallback-driven
...
- which UI issues are copy issues versus layout/hierarchy issues versus missing-state issues
